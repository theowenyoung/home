...
_mise() {
    ...
    if [[ -z ${_usage_spec_mise_2025_8_10:-} ]]; then
        _usage_spec_mise_2025_8_10="$(mise usage)"
    fi

	local cur prev words cword was_split comp_args
    _comp_initialize -n : -- "$@" || return
    # shellcheck disable=SC2207
	_comp_compgen -- -W "$(usage complete-word --shell bash -s "${_usage_spec_mise_2025_8_10}" --cword="$cword" -- "${words[@]}")"
