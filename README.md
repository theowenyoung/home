...
我用这个 Repo 来存放我所有的电脑环境和配置文件，这样我可以在任何一台电脑上快速的初始化我的环境，而不用担心丢失配置文件，或者忘记一些配置。

理论上我可以只用 Brew 或者只用 nix 来管理所有的软件和配置，但是：

- 只使用 Brew 的问题是：Brew 的命令行工具依赖管理经常很混乱，更新的时候总是不知道哪里有问题，是一个不可重现的管理工具。
- 只使用 Nix 的问题是：我的主力电脑是 macOS，很多 GUI 的软件 Nix 上并没有，而且我发现在 macOS 上搭建那样一套全 Nix 的环境，非常耗费心智，我宁愿保持 Nix 小一点，只用它来管理命令行工具，把它作为一个可重现的 mac 命令行包管理工具，这样我的心智负担是最小的。
